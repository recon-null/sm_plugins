...
{
	name = "Reserved Slots",
	author = "AlliedModders LLC | Empires Cmdr immunity added by Recon",
...
public OnPluginStart()
{
	...
	// Hook the vehicle enter event
	HookEvent("vehicle_enter", Event_VehicleEnter, EventHookMode_Post);

}

public Action:Event_VehicleEnter(Handle:event, const String:name[], bool:dontBroadcast)
{	
	...
	if (IsComm(client))
	{
		// Add commanders to the admin count
		if (g_BECmdr == -1 && g_NFCmdr == -1)
			g_adminCount += 2;
		
		// Get their team
		new team = GetClientTeam(client);
		
		// Store their client index
		if (team == 2)
			g_NFCmdr = client;
		else
			g_BECmdr = client;
...
public OnMapEnd()
{
	// Remove commanders from the admin count
	if (g_BECmdr != -1 || g_NFCmdr != -1)
		g_adminCount -= 2;
}
...
public OnClientDisconnect_Post(client)
{
	...
	// Clear the commander variable if a
	// commander disconnects
	if (client == g_NFCmdr)
		g_NFCmdr = -2;
	else if (client == g_BECmdr)
		g_BECmdr = -2;
}
