...
public Plugin:myinfo = {
	name = "TeamSwitch",
	author = "MistaGee | Logging added by Recon",
...
public Action:Command_SwitchImmed( client, args ){
	...
	if( target != -1 ){
		...
		LogAction(client, target, "\"%L\" teamswitched (immed) \"%L\"", client, target);
		}
...
public Action:Command_SwitchDeath( client, args ){
	...
	if( target != -1 ){
		...
		GetClientName( target, target_name, sizeof(target_name) );
		LogAction(client, target, "\"%L\" teamswitched (on death) \"%L\"", client, target);
		PrintToChatAll(
...
public Action:Command_SwitchRend( client, args ){
	...
	if( target != -1 ){
		...
		GetClientName( target, target_name, sizeof(target_name) );
		LogAction(client, target, "\"%L\" teamswitched (on round end) \"%L\"", client, target);
		PrintToChatAll(
